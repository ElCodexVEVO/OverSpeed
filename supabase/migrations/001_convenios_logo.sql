-- OverSpeed · migración 001 — logotipo de convenios
...
-- 1. Nueva columna. Vacía por defecto, así los convenios existentes siguen válidos.
alter table public.os_discounts add column if not exists logo_url text not null default '';

-- 2. os_manage se reemplaza entero (Postgres no permite parchear una rama suelta).
--    Único cambio respecto a schema.sql: la rama 'discount' acepta y valida logo_url.
...
-- 3. Permisos: se conservan los de schema.sql.
...
-- 4. El bucket overspeed-photos ya existe y ya es público con inserción
--    restringida a administradores, así que los logos se guardan ahí sin
--    políticas nuevas. Solo se añade webp/jpeg/png, que ya están permitidos.
